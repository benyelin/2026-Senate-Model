...
PREDICTIONS_PATH = (
    OUTPUT_DIR
    / "senate_layer1_loco_predictions.csv"
)

FOLDS_PATH = (
    OUTPUT_DIR
    / "senate_layer1_loco_fold_metrics.csv"
)
...
VALIDATION_PATH = (
    OUTPUT_DIR
    / "senate_layer1_loco_validation.txt"
)
...
def main() -> None:
    ...
    if not LAYER0_SUMMARY_PATH.exists():
    ...
    for holdout_cycle in cycles:
        ...
        test["layer1_intercept"] = coefficients[0]
        test[
            "layer1_presidential_slope"
        ] = coefficients[1]
        test[
            "layer1_generic_ballot_slope"
        ] = coefficients[2]
        test[
    ...
    layer0_summary = json.loads(
        LAYER0_SUMMARY_PATH.read_text()
    )

    layer0_metrics = layer0_summary[
        "overall_metrics"
    ]
    ...
    summary = {
        ...
        "layer0_metrics": layer0_metrics,
        "changes_from_layer0": metric_changes,
    ...
    if not validation_passed:
        raise SystemExit(
            "Layer 1 backtest validation failed."
        )
