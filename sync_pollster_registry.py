...
def sync_pollster_registry() -> int:
    ...
    if "pollster" not in polls.columns:
        raise ValueError(
            "Manual polls are missing the pollster column."
        )
    ...
    # Build a lookup from every existing canonical name and alias.
    ...
    # Preserve first-seen spelling as the initial canonical name.
        ...
        additions.append(
            {
                ...
                "house_effect_confidence": "low",
                "house_effect_notes": (
                    "Automatically added from manual polls; "
    ...
    if not additions:
        print(
            "Pollster registry already contains every "
    ...
    print(
        f"Added {len(additions)} new pollster(s):"
    )
